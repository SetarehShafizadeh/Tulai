...
tl_final <- tl_final %>%
  mutate(retouch = ifelse(str_detect(tooltypegroup, str_c(patterns, collapse = "|")), 1, retouch))
...
# Create a full summary table by joining it back to original summary table based on 'depth' and 'area'
...
pca_data <- sum_depth_sheet %>% 
  select(thickness, lithic_dens, core_freq, blank_freq, retouch_freq, tool_diversity) %>%
...
library(FactoMineR)
library(factoextra)
# Assuming 'sum_depth_sheet' is your dataframe and you have added 'm_index' as your Menhinick index
pca_data <- sum_depth_sheet[, c('thickness', 'lithic_dens', 'core_freq', 'blank_freq', 'retouch_freq', 'tool_diversity')]
