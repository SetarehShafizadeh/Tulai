...
sum_depth_sheet <- sum_depth_sheet %>%
  ...
  mutate(
    ...
    `thickness of deposit` = `thickness of deposit` / 100  # Convert from cm to m
  ) %>%
  mutate(
    volume = length * width * `thickness of deposit`  # Calculate volume in m^3
  )
...
```{r}
#WABI plot: here I do not consider those levels with a value 0 of retouch frequency. 


library(ggpmisc)

# Filter the data (get out those level with Retouch frequency=0)
...
WABI_Level <- ggplot(filtered_data,  
  ...
  geom_smooth(method = "lm", 
              ...
              linewidth = 0.5) +
  # Add text labels 
  geom_text(aes(label = paste(area, layer, level, sep = "_")), 
            vjust = 1.5, 
            hjust = 0.5, 
            check_overlap = TRUE) +
...
##calculating tool diversity is a bit tricky here; as I have some spitted depth, which we assign the number of artefacts based on the calculation of splitting, so how should I consider the tool diversity here?
```
