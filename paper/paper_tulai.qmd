...
# SS: we mainly consider ta, tp1, and d1.
knitr::kable(summary_table_area,
             caption = "Summary of lithics in all areas")
```
...
### Summary tables: chronological unit

```{r}
# Create a summary table
summary_table_unit <- tl_final %>%
  group_by(chronological_unit) %>% 
  summarise(
...
```{r}
# Generate the table
kable(summary_table_unit, 
      caption = "Summary of lithics by Chronological Units", 
      align = 'c') 

```

### Summary tables: AREA-UNIT:

```{r}
# Create a summary table based on the 'area_unit' column
summary_table_au <- tl_final %>%
  group_by(area_unit) %>%  
  summarise(
...
kable(summary_table_al, 
      caption = "Summary of lithics by Area and chronological unit") 

```

### Summary tables: Techno-Typology
...
```{r}
# Prepare data for Summary table-area_unit(au)
summary_table_au <- tl_final %>%
  filter(area %in% c("ta", "d1", "tp1")) %>% 
  group_by(area_unit) %>% 
  summarise(
...
summary_table_al <- summary_table_al %>%
  filter(!(area_unit %in% c("D1_NA", "TP1_NA")))
...
kable(
  ...
  caption = "Summary of lithics by techno-typological classification")
...
```{r}
# Create the summary table based on chronological_unit
summary_table1_unit <- tl_final %>%
  filter(area %in% c("ta", "d1", "tp1")) %>%
  group_by(chronological_unit) %>%
  summarise(
...
```{r}
# Create a summary table based on the 'depth', 'area', and 'chronological unit' columns
tl_final <- tl_final %>%
  mutate(depth = gsub("cm", "", depth))

summary_table_depth_area_unit <- tl_final %>%
  group_by(TA, depth, area, chronological_unit, grouped_level, upper, lower) %>%  
  summarise(
    `Cores (n)` = sum(!is.na(core)),  
    `Core rejuvenations pieces (n)` = sum(!is.na(corefrag)),  
    `Blade (n)` = sum(blank == "blade", na.rm = TRUE),
...
summary_table_depth_area_unit_to_join <- 
  summary_table_depth_area_unit

# Generate the table
kable(summary_table_depth_area_unit, 
      caption = "Summary of lithics by Depth, Area, and Chronological Unit")
```
...
summary_table_depth_area_unit <- summary_table_depth_area_unit %>%
  left_join(summary_table_depth_tool_type, by = c("area", "depth"))

# Remove the existing 'tool_type' if it exists
if ("tool_type" %in% names(summary_table_depth_area_unit)) {
  summary_table_depth_area_unit <- summary_table_depth_area_unit %>%
    select(-tool_type)
}
...
# Generate the table
kable(summary_table_depth_area_unit, 
      caption = "Summary of lithics by Depth, Area, and chronological_unit")
```
...
# Here, I've imported the table 'Summary by Depth, Area, chronological_unit' into Google Sheets for easier management.
...
sum_depth_sheet <- sum_depth_sheet %>%
  mutate(label = paste(area, chronological_unit, grouped_level, sep = "_"))
...
#try with summary_table_depth_area_unit
```{r}
# Create the 'label' column
summary_table_depth_area_unit <- summary_table_depth_area_unit %>%
  mutate(label = paste(area, chronological_unit, grouped_level, sep = "_"))
...
# Relocate 'label' column after 'level'
summary_table_depth_area_unit <- summary_table_depth_area_unit %>%
  relocate(label, .after = grouped_level)


------------------------------------------
# working up to here. I need to add length, width, and thickness columns
------------------------------------------
...
rownames(res.pca$ind$coord) <- sum_depth_sheet_subset$label
...
biplot_base <- fviz_pca_biplot(res.pca,
                               ...
                               label = "var",
                               addEllipses = TRUE,
                               col.ind = sum_depth_sheet_subset$area
                              )
...
res.pca$ind$coord %>% 
  as_tibble() %>% 
  mutate(labels = sum_depth_sheet_subset$label,
         area =   sum_depth_sheet_subset$area) 
