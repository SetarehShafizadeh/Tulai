...
The manuscript with figures generated here is at this URL: https://docs.google.com/document/d/1t7oICQn-sWIFlSJSL92ZVy-iKNMAE7kAJ6MvFc0vKVI/edit?usp=sharing
...
ggmap::register_stadiamaps(key = "ca45118a-734f-480b-8c3c-8d82e5a361e3")
...
ggdraw() +
  ...
  draw_plot(plot = worldplot,
            ...
            y = 0.02, # y location of inset placement
            width = .6, # Inset width
...
```{r}
# we need this pkg for the inline R code below this chunk
...
artefact_types_corrplot <- 
ggcorrplot(chisq$residuals,
           method = 'circle') + 
  scale_y_discrete(labels = function(y) str_to_upper(y)) +  
  scale_x_discrete(labels = function(x) str_to_lower(x)) +   
...
ggsave(here::here("figures/fig-chi-sq-residuals-both_plots.jpg"),
       h = 6, 
       w = 8)
...
```{r}
# BM: What are these doing here? Not appropriate for a literate programming context. These are for interactive use, in the console. 

# Set the environment variable for GDAL
# Sys.setenv(GDAL_CONFIG = "/path/to/your/conda/env/bin/gdal-config")
...
pca_data <- 
sum_depth_sheet_subset %>% 
   mutate(lithic_density = log(lithic_density + 0.001),  # Adding 1 to avoid log(0)
         core_frequency = log(core_frequency + 0.001),
         blank_frequency = log(blank_frequency + 0.001),
         retouch_frequency = log(retouch_frequency + 0.001),
         tool_diversity = log(tool_diversity + 0.001)) %>% 

  
  
  select(`lithic\ndensity` = lithic_density, 
         `core\nfrequency` = core_frequency, 
         `blank\nfrequency` = blank_frequency, 
         `retouch\nfrequency` = retouch_frequency, 
...
names(pca_data) <- str_replace_all(names(pca_data), "\n", " ")
...
```{r}
# combine PCA plots
plot_grid(biplot_modified,
          contribution_plots_panel,
          ncol = 1,
       labels = "AUTO")

ggsave(
       filename = here::here("figures/fig-pca-biplot-and-contribution.jpg"),
       h = 10, 
       w = 6)
...
A permutation MANOVA indicates that there is a significant difference in the lithic assemblage composition across the three excavation areas (`r PerMANOVA_results_inline`). We investigated this further using pairwise post-hoc tests and found that this significant difference is between TA and TP1, with the other two areas being statistically indistinguishable (@tbl-permanova)

# Correspondence Analysis (CA)

```{r}
# Example of a contingency table
lithic_data <- matrix(c(30, 20, 50, 
                        15, 25, 35,
                        45, 10, 30), 
                      nrow = 3, byrow = TRUE)
colnames(lithic_data) <- c("Cores", "Blades", "Flakes")
rownames(lithic_data) <- c("Unit1", "Unit2", "Unit3")

```

```{r}
# Perform Correspondence Analysis
res.ca <- CA(lithic_data, graph = FALSE)
...
```{r}
# Contribution of rows to Dimension 1
fviz_contrib(res.ca, choice = "row", axes = 1, top = 3)

# Contribution of rows to Dimension 2
fviz_contrib(res.ca, choice = "row", axes = 2, top = 3)
```

```{r}
# Contribution of columns to Dimension 1
fviz_contrib(res.ca, choice = "col", axes = 1, top = 3)

# Contribution of columns to Dimension 2
fviz_contrib(res.ca, choice = "col", axes = 2, top = 3)

```

```{r}
# Combine the top contributing rows for both Dim1 and Dim2
fviz_contrib(res.ca, choice = "row", axes = 1:2, top = 3)

# Combine the top contributing columns for both Dim1 and Dim2
fviz_contrib(res.ca, choice = "col", axes = 1:2, top = 3)

```

```{r}
# Save the contribution plot for rows
ggsave("contribution_plot_rows_Dim1.png", plot = fviz_contrib(res.ca, choice = "row", axes = 1, top = 3))

# Save the contribution plot for columns
ggsave("contribution_plot_cols_Dim1.png", plot = fviz_contrib(res.ca, choice = "col", axes = 1, top = 3))
