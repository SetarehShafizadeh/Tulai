...
sum_depth_sheet <- "https://docs.google.com/spreadsheets/d/1Q0QZESk412ZQLE24yPs6Rg-7Y9OMX63DHoXeIPNAIFM/edit#gid=0" 

sum_depth_sheet <- read_sheet(sum_depth_sheet)

sum_depth_sheet <- sum_depth_sheet %>%
  mutate(area_layer_level = paste0(toupper(area), "_", layer, "_", level)) %>%
  relocate(area_layer_level, .after = level) 

sum_depth_sheet <- sum_depth_sheet %>%
  mutate(retouch_freq = round(retouch_tool / total_lithics, 2),
         lithic_dens = round(total_lithics / volume, 2)
  )



write_sheet(sum_depth_sheet, ss = sheet_url)
...
```{r}
# Create the ggplot
WABI_Level <- ggplot(sum_depth_sheet, 
            aes(x = lithic_dens,
                y = retouch_freq)) +
...
```{r}
# Create the ggplot with y-axis on a log scale
WABI_Level <- ggplot(sum_depth_sheet, 
            aes(x = lithic_dens,
                y = retouch_freq)) +
  
  # Add points with different shapes based on area
